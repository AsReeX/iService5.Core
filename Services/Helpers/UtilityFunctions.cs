...
public static class UtilityFunctions
{
  ...
  internal static List<MaterialStatistics> GetMaterialList()
  {
    try
    {
      IMetadataService _metadataService = Mvx.IoCProvider.Resolve<IMetadataService>();
      List<MaterialStatistics> statisticsModules = _metadataService.GetDownloadFilesStatisticsModules();
      List<MaterialStatistics> statisticsDocuments = _metadataService.GetDownloadFilesStatisticsDocuments();
      UtilityFunctions.getDownloadedEnumbersFromDB(_metadataService);
      return statisticsModules.Concat<MaterialStatistics>((IEnumerable<MaterialStatistics>) statisticsDocuments).GroupBy(x => new
      {
        material = x.material
      }).Select<IGrouping<\u003C\u003Ef__AnonymousType0<string>, MaterialStatistics>, MaterialStatistics>(g => new MaterialStatistics()
      {
        material = g.Key.material,
        fileCount = g.Sum<MaterialStatistics>((Func<MaterialStatistics, int>) (x => x.fileCount)),
        fileSize = g.Sum<MaterialStatistics>((Func<MaterialStatistics, long>) (x => x.fileSize)),
        country = g.FirstOrDefault<MaterialStatistics>().country,
        deviceClass = g.FirstOrDefault<MaterialStatistics>().deviceClass
      }).ToList<MaterialStatistics>();
    }
    catch (Exception ex)
    {
      Mvx.IoCProvider.Resolve<ILoggingService>().getLogger().LogAppError(LoggingContext.LOCAL, ex.Message, memberName: nameof (GetMaterialList), sourceFilePath: "/Users/macmini2/bamboo-agent-home/xml-data/build-dir/UFMS-ACI54-JOB1/iService5/iService5.Core/Services/Helpers/UtilityFunctions.cs", sourceLineNumber: 700);
