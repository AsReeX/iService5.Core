...
public class MonitoringGraphicsViewModel : MvxViewModel<bool>
{
  ...
  public void OnCanvasViewPaintInstructions(Assembly assembly, SKPaintSurfaceEventArgs args)
  {
    ...
    canvas.Flush();
    SKBitmap skBitmap = SKBitmap.Decode(surface.Snapshot().Encode());
    canvas.GetLocalClipBounds(out SKRect _);
    SKColor pixel = skBitmap.GetPixel(0, 0);
    str = "#" + pixel.ToString().Substring(3);
    canvas.Clear(SKColors.White);
    canvas.Flush();
    foreach (MonitoringGraphicsViewModel.TopogramSvg renderedTopogramSvg in this.renderedTopogramSvgs)
    {
      try
      {
        string name = renderedTopogramSvg.Name;
        float xposition = renderedTopogramSvg.XPosition;
        float yposition = renderedTopogramSvg.YPosition;
        svgName = renderedTopogramSvg.SVGFileName;
        float num6;
        ...
        {
          num6 = xposition * num3;
          num7 = (float) info.Height - (yposition + 35f) * num3;
        }
        else
        {
          iService5.Core.Services.Helpers.SVGParser.SVGParser svgParser = new iService5.Core.Services.Helpers.SVGParser.SVGParser(svgName, this._loggingService);
          ...
          monitoringGraphicsSvg.ApplyTransparentColor(frameIndex, pixel);
          float tx = (xposition + monitoringGraphicsSvg.GetFrameXValue(frameIndex) + monitoringGraphicsSvg.GetXValue()) * num3;
          float ty = (float) info.Height - (float) ((double) yposition - (double) monitoringGraphicsSvg.GetFrameYValue(frameIndex) - (double) monitoringGraphicsSvg.GetYValue() + 45.0) * num3;
          Stream stream = monitoringGraphicsSvg.GetStream(frameIndex);
          Svg.Skia.SKSvg skSvg = new Svg.Skia.SKSvg();
          ...
          float width = monitoringGraphicsSvg.GetWidth();
          float height = monitoringGraphicsSvg.GetHeight();
          num6 = tx + (float) ((double) width * (double) num3 / 2.0);
          num7 = (float) ((double) ty + (double) height * (double) num3 + 35.0);
        }
        renderedTopogramSvg.LabelXPosition = num6;
        renderedTopogramSvg.LabelYPosition = num7;
        this.DisplayLabelAndValue(renderedTopogramSvg, canvas, num3);
        if (renderedTopogramSvg.NOfFrames > 1)
          this.ChangeAnimationFrame(renderedTopogramSvg);
      }
      catch (Exception ex)
      {
        this._loggingService.getLogger().LogAppError(LoggingContext.LOCAL, $"Failed to display graphics..{svgName}:{ex.Message}", memberName: nameof (OnCanvasViewPaintInstructions), sourceFilePath: "/Users/macmini2/bamboo-agent-home/xml-data/build-dir/UFMS-ACI54-JOB1/iService5/iService5.Core/ViewModels/MonitoringGraphicsViewModel.cs", sourceLineNumber: 539);
      }
    }
    canvas.Flush();
    ...
    float ratio)
  {
    try
    {
      if (string.IsNullOrEmpty(topogramSvg.NumericDisplay) || !topogramSvg.IsRendered || string.IsNullOrEmpty(topogramSvg.LabelValue))
        return;
      this._loggingService.getLogger().LogAppDebug(LoggingContext.MONITORING, $"Counter Name : {topogramSvg.DisplayName}Label to be rendered : {topogramSvg.LabelValue} passed the if condition inside the DisaplayLabelValue() function", memberName: nameof (DisplayLabelAndValue), sourceFilePath: "/Users/macmini2/bamboo-agent-home/xml-data/build-dir/UFMS-ACI54-JOB1/iService5/iService5.Core/ViewModels/MonitoringGraphicsViewModel.cs", sourceLineNumber: 747);
      ...
      double num = (double) paint2.MeasureText(topogramSvg.LabelValue, ref bounds);
      SKRect rect = bounds;
      rect.Offset(topogramSvg.LabelXPosition, topogramSvg.LabelYPosition);
      rect.Inflate(10f, 10f);
      if (topogramSvg.IsStateValueChanging)
      {
      ...
      {
        paint3.Color = SKColors.Green;
        canvas.DrawRect(rect, paint3);
      }
      canvas.DrawText(topogramSvg.LabelValue, topogramSvg.LabelXPosition + bounds.Width / 2f, topogramSvg.LabelYPosition, paint2);
      if (FeatureConfiguration.MonitoringDebugEnabled)
        this._loggingService.getLogger().LogAppDebug(LoggingContext.MONITORING, $"Counter Name : {topogramSvg.Name} Label to be rendered : {topogramSvg.LabelValue}", memberName: nameof (DisplayLabelAndValue), sourceFilePath: "/Users/macmini2/bamboo-agent-home/xml-data/build-dir/UFMS-ACI54-JOB1/iService5/iService5.Core/ViewModels/MonitoringGraphicsViewModel.cs", sourceLineNumber: 788);
      if (!string.IsNullOrEmpty(topogramSvg.DisplayName))
      {
        if (FeatureConfiguration.MonitoringDebugEnabled)
          this._loggingService.getLogger().LogAppDebug(LoggingContext.MONITORING, "Display Name to be rendered : " + topogramSvg.DisplayName, memberName: nameof (DisplayLabelAndValue), sourceFilePath: "/Users/macmini2/bamboo-agent-home/xml-data/build-dir/UFMS-ACI54-JOB1/iService5/iService5.Core/ViewModels/MonitoringGraphicsViewModel.cs", sourceLineNumber: 793);
        canvas.DrawText(this._metadataService.getMessageWithShortTextMapping($"${{{topogramSvg.DisplayName}}}"), topogramSvg.LabelXPosition, topogramSvg.LabelYPosition + rect.Height, paint1);
      }
    }
    catch (Exception ex)
    {
      this._loggingService.getLogger().LogAppError(LoggingContext.LOCAL, $"Failed to display graphics..{topogramSvg.Name}:{ex.Message}", memberName: nameof (DisplayLabelAndValue), sourceFilePath: "/Users/macmini2/bamboo-agent-home/xml-data/build-dir/UFMS-ACI54-JOB1/iService5/iService5.Core/ViewModels/MonitoringGraphicsViewModel.cs", sourceLineNumber: 801);
    }
  }
